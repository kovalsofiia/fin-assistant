...
import { 
  Check, Plus, Pencil, Trash2, RotateCcw, Calendar, Tag, FileText, 
  DollarSign, ArrowUpRight, ArrowDownLeft, Info, X, 
  TrendingUp, BarChart3, PieChart as PieChartIcon, LayoutGrid 
...
const comparisonData = computed(() => {
  const inc = getGroupedByDate('income');
  const exp = getGroupedByDate('expense');
  return {
    labels: chartLabels.value,
    datasets: [
      { label: 'Доходи', data: chartLabels.value.map(d => inc[d] || 0), backgroundColor: '#22c55e', borderRadius: 8 },
      { label: 'Витрати', data: chartLabels.value.map(d => exp[d] || 0), backgroundColor: '#ef4444', borderRadius: 8 }
    ]
  };
});
...
<template>
  <div class="max-w-6xl mx-auto p-4 sm:p-8 animate-fade-in font-sans">
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 sm:mb-12">
      ...
      <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
        <button 
          ...
          class="w-full md:w-auto px-6 py-4 rounded-2xl font-black transition-all flex items-center justify-center gap-2"
          :class="showCharts ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-gray-700 border-2 border-gray-100 hover:bg-gray-50'"
        >
    ...
    >
      <div v-if="showCharts && !store.isLoading" class="mb-12 space-y-8">
        <!-- Top Row: Doughnuts -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <!-- Expenses Doughnut -->
          <div class="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-100 relative group">
        ...
        <!-- Bottom Row: Trends -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div class="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-100">
            <h3 class="text-xl font-black text-gray-800 mb-6 flex items-center gap-3"><TrendingUp class="text-blue-500" /> Динаміка витрат</h3>
            <div class="h-64"><Line :data="expenseTrendData" :options="areaOptions('expense')" /></div>
          </div>
          <div class="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-100">
            ...
            <div class="h-64"><Line :data="categoryTrendData" :options="areaOptions('category')" /></div>
          </div>
        </div>
      </div>
    </transition>
